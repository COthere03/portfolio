...
import { Briefcase, Code, User, Download, Calendar, Sparkles, Target, Mail, Star, Network } from 'lucide-react';
...
export const AboutSection = () => {
  const [activeTab, setActiveTab] = useState('CareerObjective');
  const [mousePosition, setMousePosition] = useState({ x: 0, y: 0 });
  const [counter, setCounter] = useState(0);

  const achievements = [
    { number: "Software Developer", label: "Service", icon: <Briefcase className="h-5 w-5" />, suffix: "" },
    { number: "BS Computer Science", label: "University of Makati", icon: <Calendar className="h-5 w-5" />, suffix: "" },
    { number: "Taguig City", label: "Location", icon: <Target className="h-5 w-5" />, suffix: "" },
    { number: "Client-based Projects", label: "Features", icon: <User className="h-5 w-5" />, suffix: "" }
  ];
  ...
  const socialLinks = [
    { icon: <Network className="h-5 w-5" />, href: "https://www.linkedin.com/in/asilito-caasi-6b853938a", name: "LinkedIn" },
    { icon: <Code className="h-5 w-5" />, href: "https://github.com/COthere03", name: "GitHub" },
    { icon: <Mail className="h-5 w-5" />, href: "https://mail.google.com/mail/?view=cm&fs=1&to=asicaasi28@gmail.com", name: "Email" }
  ];

  const tabContent = {
    CareerObjective: <em>"To create innovative software solutions that simplify challenges, empower clients, and drive success."</em>
  };
  ...
  const handleDownload = () => {
    ...
    link.href = '/CAASI_CV.pdf'; // Must be in public folder
    link.download = 'CAASI_CV.pdf';
                ...
                <div className="flex flex-col md:flex-row items-center gap-6 md:gap-8">
                  ...
                  <div className="relative flex-shrink-0">
                    <div className="w-28 h-28 sm:w-32 sm:h-32 rounded-2xl overflow-hidden border-4 border-primary/20 shadow-2xl transition-all duration-500 group-hover:border-primary/40 group-hover:scale-105 md:group-hover:scale-110 relative">
                      <img src="/profile-logo.jfif" alt="MD Sahil" className="w-full h-full object-cover" />
                  ...
                  <div className="flex-1 text-center md:text-left">
                    <div className="flex flex-col sm:flex-row items-center justify-between gap-3 mb-4 sm:mb-6">
                      <h2 className="text-2xl sm:text-3xl font-bold">ASILITO CAASI</h2>
                      <motion.div className="flex w-fit items-center gap-2 px-4 py-2 rounded-full bg-primary/10 border border-primary/20 text-primary text-sm font-medium backdrop-blur-sm" variants={{ hidden: { y: 30, opacity: 0 }, visible: { y: 0, opacity: 1, transition: { duration: 0.8 } } }}>
                ...
                <div className="min-h-[100px] sm:min-h-[120px] flex items-center justify-center">
                  <AnimatePresence mode="sync">
                    <motion.p
                      ...
                      className="text-sm sm:text-base md:text-lg text-muted-foreground leading-relaxed text-center"
                    >
              ...
              <div className="mt-4 sm:mt-6 p-3 sm:p-4 bg-background/50 rounded-xl border border-border">
                ...
                <div className="flex flex-wrap justify-center gap-2 sm:gap-4">
                  {socialLinks.map((social, index) => (
                    <a key={index} href={social.href} className="group flex items-center gap-2 p-2 bg-background rounded-lg text-muted-foreground hover:text-primary hover:bg-primary/10 transition-all duration-300 hover:scale-110">
                      {social.icon}
                      <span className="hidden group-hover:inline-block text-xs sm:text-sm">{social.name}</span>
                    </a>
                  ))}
                </div>
              </div>
